<template>
  <div class="fun-list-box" id="fun_list_box" :style="{height: `${height}px`}">
      <flexbox :gutter="0" v-for="(list, index) in components" :key="index">
        <flexbox-item :span="1/3" v-for="component in list" :key="component.name" class="cbox vux-1px-t vux-tap-active" @click.native="go(component.name.toLowerCase())">
          <div class="vux-1px-r cbox-inner">
            <span class="ib-icon ib-icon-big" v-html="component.icon" :style="{color: component.color}"></span>
            <br>
            <span :style="{fontSize: component.title.length > 12 ? '12px' : ''}">{{component.title}}</span>
          </div>
        </flexbox-item>
      </flexbox>
    </div>
...
import components from '@/datas/component_list.json'
...
export default {
  ...
  activated () {
    document.querySelector('#fun_list_box').scrollTop = this.funTop
  },
  methods: {
    go (name) {
