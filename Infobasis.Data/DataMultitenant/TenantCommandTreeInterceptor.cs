...
namespace Infobasis.Data.DataMultitenant
{
    ...
    public class TenantCommandTreeInterceptor : IDbCommandTreeInterceptor
    {
            ...
            if (interceptionContext.OriginalResult.DataSpace == DataSpace.SSpace)
            {
                ...
                if (string.IsNullOrEmpty(userIdclaim))
                {
                    ...
                    if (userClaimFromClaims != null)
                        userIdclaim = userClaimFromClaims.Value;
                }
                if (userIdclaim == null)
                {
                    return;
                ...
                string[] companyAndUserName = userIdclaim.Split(',');
                var companyId = Infobasis.Data.DataAccess.Change.ToInt(companyAndUserName[0]);
                if (companyId == 0)
                    return;
                ...
                if (InterceptInsertCommand(interceptionContext, companyId))
                {
                    return;
                }

                if (InterceptUpdate(interceptionContext, companyId))
                {
                    return;
                }

                InterceptDeleteCommand(interceptionContext, companyId);
            }
        }
        ...
        /// </summary>
        private static bool InterceptInsertCommand(DbCommandTreeInterceptionContext interceptionContext, int companyId)
        {
            ...
            if (insertCommand != null)
            {
                ...
                if (!string.IsNullOrEmpty(column))
                {
                    ...
                    var tenantSetClause =
                        DbExpressionBuilder.SetClause(tenantProperty, DbExpression.FromInt32(companyId));
        ...
        /// </summary>
        private static bool InterceptUpdate(DbCommandTreeInterceptionContext interceptionContext, int companyId)
        {
            ...
            if (updateCommand != null)
            {
                ...
                if (!string.IsNullOrEmpty(column))
                {
                    ...
                    var tenantIdWherePredicate = DbExpressionBuilder.Equal(tenantProperty, DbExpression.FromInt32(companyId));
        ...
        /// </summary>
        private static void InterceptDeleteCommand(DbCommandTreeInterceptionContext interceptionContext, int companyId)
        {
