...
using Infobasis.Web.Controls;

namespace Infobasis.Web
{
    public class PageBase : System.Web.UI.Page, IBindingHelper
    {
        ...
        public static InfobasisContext DB
        {
            get
            {
                // http://stackoverflow.com/questions/6334592/one-dbcontext-per-request-in-asp-net-mvc-without-ioc-container
                if (!HttpContext.Current.Items.Contains("__InfobasisContext"))
                {
                    HttpContext.Current.Items["__InfobasisContext"] = new InfobasisContext();
                }
                return HttpContext.Current.Items["__InfobasisContext"] as InfobasisContext;
            }
        }
        ...
        public static UnitOfWork UnitOfWork
        {
            get
            {
                if (!HttpContext.Current.Items.Contains("__InfobasisUnitOfWork"))
                {
                    HttpContext.Current.Items["__InfobasisUnitOfWork"] = new UnitOfWork();
                }
                return HttpContext.Current.Items["__InfobasisUnitOfWork"] as UnitOfWork;            
            }
        }
        ...
        {
            Infobasis.Web.Util.AntiCsrf.Attach(this);
        }
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            companyID = UserInfo.Current != null ? UserInfo.Current.CompanyID : 0;
            if (pm != null)
            {
        ...
        protected void ScrollToSelectedRow(Grid grid)
        {
            int scrollid = GetQueryIntValue("scrollid");
            scrollid = scrollid < 0 ? 0 : scrollid;
            grid.SelectedRowIndex = scrollid;
        }
        ...
        public string HtmlEncode(object value)
        {
            if (value == null)
                return null;
            return Infobasis.Web.Util.StringUtil.HtmlEncode(value.ToString());
        }
        ...
        protected bool SaveChanges()
        {
            ...
            catch (DbUpdateException dbEx)
            {
                if (dbEx.InnerException != null && dbEx.InnerException.InnerException != null)
                    msg = dbEx.InnerException.InnerException.Message;
                else
                    msg = dbEx.InnerException.Message;
            }
            catch (NotSupportedException dbEx)
            {
                msg = dbEx.Message;
            }
            LogUtils.Error(msg);
        ...
        /// <returns></returns>
        protected string GenerateNum(string numType)
        {
            return GenerateNum(numType, true);
        }
        /// <summary>
        /// 生成编号
        /// </summary>
        /// <param name="numType"></param>
        /// <returns></returns>
        protected string GenerateNum(string numType, bool hasDate)
        {
            int companyID = UserInfo.Current.CompanyID;
            DateTime currentDate = DateTime.Now;
            SystemNum systemNum = null;
            if (hasDate)
            {
                systemNum = DB.SystemNums.Where(item => item.NumType == numType
                    && item.CompanyID == companyID
                    && item.LastUpdateDatetime.Value.Year == currentDate.Year
                    && item.LastUpdateDatetime.Value.Month == currentDate.Month
                    && item.LastUpdateDatetime.Value.Day == currentDate.Day).FirstOrDefault();
            }
            else
            {
                systemNum = DB.SystemNums.Where(item => item.NumType == numType
                    && item.CompanyID == companyID).FirstOrDefault();            
            }
            int rtnNum = 1;
            if (systemNum == null)
            {
                SystemNum newSystemNum = new SystemNum()
                {
                    NumType = numType,
                    Num = 1,
                    CompanyID = companyID,
                ...
                DB.SystemNums.Add(newSystemNum);
                DB.SaveChanges();
                rtnNum = 1;
            }
            else
            {
                rtnNum = systemNum.Num + 1;
                systemNum.LastUpdateDatetime = currentDate;
                systemNum.Num = rtnNum;
                DB.SaveChanges();
            }

            if (hasDate)
                return numType + String.Format("{0}-{1}", currentDate.ToString("yyyyMMdd"), rtnNum.ToString("0000"));
            else
                return numType + String.Format("{0}", rtnNum.ToString("0000"));
        }
        ...
        protected void ResolveDeleteMenuButtonForGrid(FineUIPro.MenuButton btn, Grid grid)
        {
            ResolveDeleteMenuButtonForGrid(btn, grid, DELETE_CONFIRM_STRING);
        }

        protected void ResolveDeleteMenuButtonForGrid(FineUIPro.MenuButton btn, Grid grid, string confirmTemplate)
        {
            ResolveDeleteMenuButtonForGrid(btn, grid, NO_SELECTION_STRING, confirmTemplate);
        }

        protected void ResolveDeleteMenuButtonForGrid(FineUIPro.MenuButton btn, Grid grid, string noSelectionMessage, string confirmTemplate)
        {
            // 点击删除按钮时，至少选中一项
            btn.OnClientClick = grid.GetNoSelectionAlertInParentReference(noSelectionMessage);
            btn.ConfirmText = String.Format(confirmTemplate, "&nbsp;<span class=\"highlight\"><script>" + grid.GetSelectedCountReference() + "</script></span>&nbsp;");
        ...
        #region Implementation of IBindingHelper

        public object Eval(object container, string fieldName)
        {
            return BindingHelper.Eval(container, fieldName);
        }

        public object Eval(string format, object container, params string[] fieldNames)
        {
            return BindingHelper.Eval(format, container, fieldNames);
        }

        //=======================================================================
        public object BindKey(object container)
        {
            return BindingHelper.BindKey(container);
        }

        public object BindKey(string format, object container)
        {
            return BindingHelper.BindKey(format, container);
        }

        //=======================================================================
        public bool IsLastRow(object container)
        {
            return BindingHelper.IsLastRow(container);
        }
        ...
        protected DataTable GetEntityListTable(string code)
        { 
            IInfobasisDataSource db = InfobasisDataSource.Create();
            DataTable table = db.ExecuteTable(@"SELECT SYtbEntityListValue.ID, SYtbEntityListValue.Name, SYtbEntityListValue.Code, SYtbEntityListValue.Value FROM SYtbEntityListValue
                INNER JOIN SYtbEntityList
                ON SYtbEntityList.ID = SYtbEntityListValue.EntityListID
                WHERE SYtbEntityList.Code = @Code
                AND SYtbEntityListValue.IsActive = 1
                AND SYtbEntityList.CompanyID = @CompanyID
                ORDER BY SYtbEntityListValue.DisplayOrder", code, companyID);
