...
namespace Infobasis.Web.Util
{
    ...
    {
        private ChineseConvertToLetter()
        {
        ...
        /// 判断是否为中文字符
        /// </summary>
        /// <param name="CnChar">单个字符</param>
        /// <returns>是否中文字符</returns>
        private static bool IsChineseCharacters(string CnChar)
        {
        ...
        private static String GetSpellCode(String CnStr)
        {
            ...
            for (int i = 0; i < CnStr.Length; i++)
                _Temp = _Temp + GetCharSpellCode(CnStr.Substring(i, 1));
            return _Temp;
        }
        ...
        private static string GetCharSpellCode(string CnChar)
        {
            ...
            System.Text.Encoding deft = System.Text.Encoding.Default;

            byte[] ZW = System.Text.Encoding.GetEncoding("Chinese").GetBytes(CnChar); 
            ...
            else return GetSpecialSpellCode(CnChar);
        }


        private static string GetSpecialSpellCode(string CnChar)
        {
            string resultCode = "?";

            int pos = Array.IndexOf(otherChinese, CnChar);
            if (pos != decimal.MinusOne)
            {
                resultCode = otherPinYin[pos][0].ToString();
            }
